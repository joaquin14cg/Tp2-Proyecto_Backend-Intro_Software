...
def validar_email(email)->str:
    email = ut.validar_string_no_vacio(email, 'email')
    if not re.match(const.PATRON_EMAIL, email):
...
def validar_nombre_o_apellido(valor, nombre_campo: str)->str:
    valor = ut.validar_string_no_vacio(valor, nombre_campo)
    if not re.match(const.PATRON_NOMBRE, valor):
        raise ValueError(ut.construir_error_api(
...
def validar_body_socio(body:dict)->dict:
    ...
    for campo in const.CAMPOS_SOCIO:
        try:
...
def validar_id_socio(id_str)->int:
    return ut.validar_minimo(ut.validar_entero(id_str, 'id'), const.MIN_ID, 'id')
